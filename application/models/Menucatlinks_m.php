...
class Menucatlinks_m extends MY_Model
{
    protected $_table_name = 'tbl_menu_cat_links';
    protected $_order_by = 'volgorde';
    protected $_primary_key = 'link_id';

    public function get_new ()
    {
        $menucatlink = new stdClass();
        $menucatlink->cat_id = '';
        $menucatlink->item_id = '';
        $menucatlink->type = '';
        $menucatlink->volgorde = '';
        return $menucatlink;
    }

    public function get_all ()
    {
        $this->db->select('link_id, cat_id');
    ...
    public function get_all_items($cat_id)
    {
        $this->db->order_by($this->_order_by);
        $query = $this->db->get_where($this->_table_name, array('cat_id' => $cat_id));
    ...
    public function delete_link($cat_id, $item_id)
    {
        $this->db->delete($this->_table_name, array('cat_id' => $cat_id, 'item_id' => $item_id));
    }
    ...
    public function get_max_volgorde($cat_id)
    {
    ...
    public function get_item_name($link_id)
    {
        ...
        $query = $this->db->get_where($type->tabel, array($type->field_id => $item_info->item_id), '', '');
        $info = $query->result();
        $info = $info[0];
